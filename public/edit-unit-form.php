...
if (isset($_POST['btnEdit'])) {
    ...
    if ($permissions['categories']['update'] == 1) {
        ...
        if (!empty($name) && !empty($short_code)) {
            $sql_query = "UPDATE unit SET 	name = ' $name',  short_code = '$short_code',parent_id = '$parent_id' , conversion = '$conversion' , type = '$type' WHERE id =  $ID";
            if ($db->sql($sql_query)) {
                $db->sql($sql_query);
            ...
            <div class="box box-primary">
                ...
                <form id="edit_unit_form" method="post" enctype="multipart/form-data">
                    <div class="box-body">
                        ...
                        <div class="form-group">
                            <label class="radio-inline"><input type="radio" name="type" id="unit" value="unit" <?php echo ($res[0]['type']=='unit')?'checked':'' ?>>Unit</label>
                            <label class="radio-inline"><input type="radio" name="type" id="subunit" value="subunit" <?php echo ($res[0]['type']=='subunit')?'checked':'' ?>>Sub unit</label>
                        </div>
                        <div class="form-group">
