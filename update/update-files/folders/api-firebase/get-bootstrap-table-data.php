...
if (isset($_GET['table']) && $_GET['table'] == 'products') {
    ...
    foreach ($res as $row) {
        ...
        if ($row['indicator'] == 1) {
            $indicator = "<span class='label label-success'>physical</span>";
        }
        if ($row['indicator'] == 2) {
            $indicator = "<span class='label label-danger'>softcopy</span>";
        }
...
if (isset($_GET['table']) && $_GET['table'] == 'users') {
    ...
    $sql = "SELECT COUNT(u.id) as total FROM `users` u LEFT JOIN user_addresses ua on u.id=ua.user_id LEFT JOIN pincodes p on p.id=ua.pincode_id LEFT JOIN area a on a.id=ua.area_id LEFT JOIN cities c on c.id=a.city_id  " . $where ;
    $db->sql($sql);
    $res = $db->getResult();
    foreach ($res as $row)
        $total = $row['total'];
...
if (isset($_GET['table']) && $_GET['table'] == 'area') {
    ...
    if (isset($_GET['search']) && !empty($_GET['search'])) {
        $search = $db->escapeString($fn->xss_clean($_GET['search']));
        if (isset($_GET['filter_area']) && !empty($_GET['filter_area'])) {
            $where .= " and a.`id` like '%" . $search . "%' OR a.`name` like '%" . $search . "%' OR `pincode_id` like '%" . $search . "%' OR c.`pincode` like '%" . $search . "%'";
        } else {
            $where .= " Where a.`id` like '%" . $search . "%' OR a.`name` like '%" . $search . "%' OR `pincode_id` like '%" . $search . "%' OR c.`pincode` like '%" . $search . "%'";
        }
    }
