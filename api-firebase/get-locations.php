...
if (isset($_POST['get_pincodes']) && $_POST['get_pincodes'] == 1) {
    ...
    */
    ...
    $where1 = "";
    if (isset($_POST['id']) && !empty($_POST['id']) && is_numeric($_POST['id'])) {
        $where .=  !empty($where) ? " AND p.`id`=" . $id : " AND p.`id`=" . $id;
        $where1 .=  !empty($where1) ? " AND p.`id`=" . $id : " AND p.`id`=" . $id;
    }
    if (isset($_POST['area_id']) && !empty($_POST['area_id']) && is_numeric($_POST['area_id'])) {
        $where .=  !empty($where) ? " AND a.`id`=" . $area_id : " AND a.`id`=" . $area_id;
    }

    if (!isset($_POST['area_id'])) {
        $sql = "SELECT count(p.id) as total FROM pincodes p WHERE p.status = 1 $where1";
    } else {
        $sql = "SELECT count(p.id) as total FROM pincodes p RIGHT JOIN area a ON p.id=a.pincode_id WHERE p.status = 1 $where";
    }
    $db->sql($sql);
    $total = $db->getResult();

    $sql = "SELECT p.* FROM pincodes p JOIN area a ON p.id=a.pincode_id where p.status = 1 $where  GROUP BY p.id ORDER BY $sort $order  ";
    $db->sql($sql);
    $res = $db->getResult();
    $product = array();
